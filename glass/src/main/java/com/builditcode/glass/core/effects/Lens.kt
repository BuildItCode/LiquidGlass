...
import com.builditcode.glass.core.helpers.RoundedRectRefractionShaderString
import com.builditcode.glass.core.helpers.RoundedRectRefractionWithDispersionShaderString
import com.builditcode.glass.core.helpers.RuntimeShaderEffect
import com.builditcode.glass.core.isRuntimeShaderSupported
import com.builditcode.glass.core.shapes.RoundedRectangularShape
    ...
    get() = when (val shape = shape) {
        ...
        is CornerBasedShape -> {
            ...
            if (outline is Outline.Rounded) {
                ...
                floatArrayOf(
                    roundRect.topLeftCornerRadius.x.fastCoerceAtMost(size.minDimension / 2f),
                    roundRect.topRightCornerRadius.x.fastCoerceAtMost(size.minDimension / 2f),
                    roundRect.bottomRightCornerRadius.x.fastCoerceAtMost(size.minDimension / 2f),
                    roundRect.bottomLeftCornerRadius.x.fastCoerceAtMost(size.minDimension / 2f)
                )
            } else {
                null
            }
...
private fun throwUnsupportedSDFException(): Nothing {
    throw UnsupportedOperationException(
        "Only rounded shapes are supported in lens effects."
    )
}
